...
# or a key pasted into a document. This looks at what is actually tracked. Folio's supporter key and the key that
# signs a package source are the only two things that make Folio's signatures worth anything: with one of them,
# anyone can mint supporter codes or publish packages as Folio.
set -uo pipefail
cd "$(dirname "$0")/.."
